...
internal static class YieldInstructionCache // 데이터 저장용이라 MonoBehaviour가 필요없음. 대신 internal로 싱글톤 비슷한 효과를 냄.
{
    ...
    public static WaitForSecondsRealtime WaitForSecondsRealtime(float second)
    {
        ...
        if (!waitForSecondsRealtime.TryGetValue(second, out wfs)) // Dictionary에 일치하는 키 값이 있지 않을 경우(있으면 (out wfs)으로 Dictionary 안의 값을 리턴함.)
        {
            waitForSecondsRealtime.Add(second, wfs = new WaitForSecondsRealtime(second)); // 입력된 키 값으로 새 객체를 만들어 Dictionary에 저장함.
        }
        return wfs;
    }
